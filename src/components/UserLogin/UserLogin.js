...
import logo from "../../images/rocketfilms.png";
...
class UserLogIn extends Component {
  ...
  componentDidMount() {
    this.props.getRequestToken(
      `https://api.themoviedb.org/3/authentication/token/new?api_key=${this.props.apiKey}`
    );
  }

  // Creates guest session
  handleBrowseAsGuest = () => {
    this.props.getGuestSession(
      `https://api.themoviedb.org/3/authentication/guest_session/new?api_key=${this.props.apiKey}`
    );
      ...
      <div className="user-log-in">
        <div className="user-log-in-container">
          <header className="user-log-in-container-header">
            <img
              className="user-log-in-container-header__logo"
              src={logo}
              alt="logo"
            />
          ...
          <div className="user-log-in-container-content">
            <a
              href={`https://www.themoviedb.org/authenticate/${this.props.requestToken.request_token}?redirect_to=http://localhost:3000/profile/approved`}
            >
              <button
                className="user-log-in-container-content__button"
          ...
          <p className="user-log-in-container-content__warning">
            Guest users have access to limited features
          </p>
        </div>

        <div className="user-log-in-side-image">
          <h2 className="user-log-in-side-image__title">
            Browse and rate your favorite shows
          </h2>
        </div>
      </div>
    );
